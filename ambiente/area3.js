...
function createArea3(scene, chave, objetosColidiveis, rampas, texturas) {
    ...
    //----------------- PORTÃO E SUA LÓGICA ----------------//
        ...
        function updateArea3(chaveColetada) {
            ...
            if (!alreadyAnimate && !animating && chaveColetada) {
                areaAtivacao.getWorldPosition(tmpWorldPos);
                const d = scene.personagem.position.distanceTo(tmpWorldPos);
                chave.position.set(tmpWorldPos.x, tmpWorldPos.y, tmpWorldPos.z);
                if (d < rAtivacao) {
                    animating = true;
                    moveProgress = 0;
                    chave.visible = true;
    ...
    objetosColidiveis.push(paredeNorte, paredeOeste, paredeLeste,
        paredesSul1, paredesSul2, base, areaAlta1, areaAlta2,
        corrimao1, corrimao2, corrimaoFundo
    );
